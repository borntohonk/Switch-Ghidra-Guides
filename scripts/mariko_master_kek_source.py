...
with open('temp.keys', 'w') as temp_keys:
    ...
    with open(f'{firmware}/titleid//0100000000000819/romfs/a/pkg1/Decrypted.bin', 'rb') as decrypted_bin:
        ...
        if mariko_master_kek_source_key == key_sources.mariko_master_kek_sources[-1]:
            ...
            print(formatted_master_kek_source)
            ...
            print(f'^ add this string to master_key_sources_dev array ^')
            print(formatted_mariko_master_kek_source_dev)
            print(f'^ unused, but output for consistency ^') # https://github.com/Atmosphere-NX/Atmosphere/blob/master/fusee/program/source/fusee_key_derivation.cpp#L31
        decrypted_bin.close()
        os.remove('temp.keys')
