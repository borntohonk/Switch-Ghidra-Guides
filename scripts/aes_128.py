# Pure python AES128 implementation
# SciresM, 2017-2026
import sys
from struct import unpack as up, pack as pk
from binascii import hexlify as hx, unhexlify as uhx

try:
    from Cryptodome.Cipher import AES
    from Cryptodome.Util import Counter
except ModuleNotFoundError:
    try:
        from Crypto.Cipher import AES
        from Crypto.Util import Counter
    except ModuleNotFoundError:
        print('Please install pycryptodome(ex) first!')
        sys.exit(1)

# Constants
BLOCK_SIZE = 0x10
SECTOR_SIZE_DEFAULT = 0x200
NUM_ROUNDS_128 = 10
TWEAK_XTS_CARRY_BIT = 1 << 128
TWEAK_XTS_MODULUS = (1 << 128) | 0x87

def sxor(s1, s2):
	"""XOR two byte strings of equal length."""
	assert(len(s1) == len(s2))
	return b''.join([pk('B', x ^ y) for x, y in zip(s1, s2)])

def _left_shift_tweak(tweak_bytes):
	"""Perform left shift on tweak with GF(2^128) reduction."""
	_t = int(hx(tweak_bytes[::-1]), 16)
	_t <<= 1
	if _t & TWEAK_XTS_CARRY_BIT:
		_t ^= TWEAK_XTS_MODULUS
	return uhx('%032X' % _t)[::-1]

class AESCBC:
	'''Class for performing AES CBC cipher operations.'''

	def __init__(self, key, iv):
		self.aes = AESECB(key)
		if len(iv) != self.aes.block_size:
			raise ValueError('IV must be of size %X!' % self.aes.block_size)
		self.iv = iv

	def encrypt(self, data, iv=None):
		'''Encrypts some data in CBC mode.'''
		if iv is None:
			iv = self.iv
		out = b''
		while data:
			encb = self.aes.encrypt_block_ecb(sxor(data[:0x10], iv))
			out += encb
			iv = encb
			data = data[0x10:]
		return out

	def decrypt(self, data, iv=None):
		'''Decrypts some data in CBC mode.'''
		if len(data) % self.aes.block_size:
			raise ValueError('Data is not aligned to block size!')
		if iv is None:
			iv = self.iv
		out = b''
		while data:
			decb = sxor(self.aes.decrypt_block_ecb(data[:0x10]), iv)
			out += decb
			iv = data[:0x10]
			data = data[0x10:]
		return out

	def set_iv(self, iv):
		if len(iv) != self.aes.block_size:
			raise ValueError('IV must be of size %X!' % self.aes.block_size)
		self.iv = iv

class AESCTR:
	'''Class for performing AES CTR cipher operations.'''

	def __init__(self, key, nonce, offset=0):
		self.key = key
		self.nonce = nonce
		self.seek(offset)

	def encrypt(self, data, ctr=None):
		if ctr is None:
			ctr = self.ctr
		return self.aes.encrypt(data)

	def decrypt(self, data, ctr=None):
		return self.encrypt(data, ctr)

	def seek(self, offset):
		self.ctr = Counter.new(64, prefix=self.nonce[0:8], initial_value=(offset >> 4))
		self.aes = AES.new(self.key, AES.MODE_CTR, counter=self.ctr)

	def bktrPrefix(self, ctr_val):
		return self.nonce[0:4] + ctr_val.to_bytes(4, 'big')

	def bktrSeek(self, offset, ctr_val, virtualOffset=0):
		offset += virtualOffset
		self.ctr = Counter.new(64, prefix=self.bktrPrefix(ctr_val), initial_value=(offset >> 4))
		self.aes = AES.new(self.key, AES.MODE_CTR, counter=self.ctr)

class AESXTS:
	'''Class for performing AES XTS cipher operations'''

	def __init__(self, keys, sector=0):
		self.keys = keys[:16], keys[16:]
		if not(isinstance(self.keys, tuple) and len(self.keys) == 2):
			raise TypeError('XTS mode requires a tuple of two keys.')
		self.K1 = AESECB(self.keys[0])
		self.K2 = AESECB(self.keys[1])
		self.sector = sector
		self.block_size = self.K1.block_size
		self.sector_size = SECTOR_SIZE_DEFAULT

	def encrypt(self, data, sector=None):
		"""Encrypt data in XTS mode (standard)."""
		if sector is None:
			sector = self.sector
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		while data:
			tweak = self.get_tweak(sector)
			out += self.encrypt_sector(data[:self.sector_size], tweak)
			data = data[self.sector_size:]
			sector += 1
		return out

	def encrypt_sector(self, data, tweak):
		"""Encrypt a single sector using standard XTS (with K1.encrypt)."""
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		tweak = self.K2.encrypt(uhx('%032X' % tweak))
		while data:
			out += sxor(tweak, self.K1.encrypt(sxor(data[:BLOCK_SIZE], tweak)))
			tweak = _left_shift_tweak(tweak)
			data = data[BLOCK_SIZE:]
		return out

	def decrypt(self, data, sector=None):
		"""Decrypt data in XTS mode (standard)."""
		if sector is None:
			sector = self.sector
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		while data:
			tweak = self.get_tweak(sector)
			out += self.decrypt_sector(data[:self.sector_size], tweak)
			data = data[self.sector_size:]
			sector += 1
		return out

	def decrypt_sector(self, data, tweak):
		"""Decrypt a single sector using standard XTS (with K1.decrypt)."""
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		tweak = self.K2.encrypt(uhx('%032X' % tweak))
		while data:
			a = self.K1.decrypt(sxor(data[:BLOCK_SIZE], tweak))
			out += sxor(tweak, a)
			tweak = _left_shift_tweak(tweak)
			data = data[BLOCK_SIZE:]
		return out

	def get_tweak(self, sector=None):
		"""Convert sector number to XTS tweak value."""
		if sector is None:
			sector = self.sector
		tweak = 0
		for i in range(self.block_size):
			tweak |= (sector & 0xFF) << (i * 8)
			sector >>= 8
		return tweak

	def set_sector(self, sector):
		"""Set the current sector for XTS operations."""
		self.sector = sector

class AESXTSN:
	'''Class for performing Nintendo AES XTS cipher operations (uses encrypt_block_ecb/decrypt_block_ecb)'''

	def __init__(self, keys, sector_size=SECTOR_SIZE_DEFAULT, sector=0):
		if not(isinstance(keys, tuple) and len(keys) == 2):
			raise TypeError('XTS mode requires a tuple of two keys.')
		self.K1 = AESECB(keys[0])
		self.K2 = AESECB(keys[1])
		self.keys = keys
		self.sector = sector
		self.sector_size = sector_size
		self.block_size = self.K1.block_size

	def encrypt(self, data, sector=None):
		"""Encrypt data in Nintendo XTS mode."""
		if sector is None:
			sector = self.sector
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		while data:
			tweak = self.get_tweak(sector)
			out += self.encrypt_sector(data[:self.sector_size], tweak)
			data = data[self.sector_size:]
			sector += 1
		return out

	def encrypt_sector(self, data, tweak):
		"""Encrypt a single sector using Nintendo XTS (with K1.encrypt_block_ecb)."""
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		tweak = self.K2.encrypt(uhx('%032X' % tweak))
		while data:
			out += sxor(tweak, self.K1.encrypt_block_ecb(sxor(data[:BLOCK_SIZE], tweak)))
			tweak = _left_shift_tweak(tweak)
			data = data[BLOCK_SIZE:]
		return out

	def decrypt(self, data, sector=None):
		"""Decrypt data in Nintendo XTS mode."""
		if sector is None:
			sector = self.sector
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		while data:
			tweak = self.get_tweak(sector)
			out += self.decrypt_sector(data[:self.sector_size], tweak)
			data = data[self.sector_size:]
			sector += 1
		return out

	def decrypt_sector(self, data, tweak):
		"""Decrypt a single sector using Nintendo XTS (with K1.decrypt_block_ecb)."""
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		tweak = self.K2.encrypt(uhx('%032X' % tweak))
		while data:
			out += sxor(tweak, self.K1.decrypt_block_ecb(sxor(data[:BLOCK_SIZE], tweak)))
			tweak = _left_shift_tweak(tweak)
			data = data[BLOCK_SIZE:]
		return out

	def get_tweak(self, sector=None):
		'''Gets tweak for use in XEX.'''
		if sector is None:
			sector = self.sector
		tweak = 0
		for i in range(self.block_size):
			tweak |= (sector & 0xFF) << (i * 8)
			sector >>= 8
		return tweak

	def set_sector(self, sector):
		"""Set the current sector for XTS operations."""
		self.sector = sector

	def set_sector_size(self, sector_size):
		"""Set the sector size for XTS operations."""
		self.sector_size = sector_size

class AESECB:
	'''Class for performing AES ECB cipher operations.'''

	# Constants for performing AES operations -- rcon table, S boxes.
	rcon_table = [
		]
	sbox_enc = []

	sbox_dec = []

	mult_1_table = range(0x100)
	mult_2_table = [
		]
	mult_3_table = [
		]

	mult_9_table = [
		]
	mult_B_table = [
		]
	mult_D_table = [
		]
	mult_E_table = [
		]

	mult_table = [None, mult_1_table, mult_2_table, mult_3_table, None, None, None, None,
				  None, mult_9_table, None, mult_B_table, None, mult_D_table, mult_E_table, None]

	mix_mults = [[0x2, 0x3, 0x1, 0x1], [0x1, 0x2, 0x3, 0x1], [0x1, 0x1, 0x2, 0x3], [0x3, 0x1, 0x1, 0x2]]
	unmix_mults = [[0xE, 0xB, 0xD, 0x9], [0x9, 0xE, 0xB, 0xD], [0xD, 0x9, 0xE, 0xB], [0xB, 0xD, 0x9, 0xE]]

	def __init__(self, key):
		self.block_size = BLOCK_SIZE
		self.num_rounds = NUM_ROUNDS_128
		if len(key) != self.block_size:
			raise ValueError('Key must be of size %X!' % self.block_size)
		self.keys = [list(up('>IIII', key))]
		for i in range(1, self.num_rounds + 1):
			new_key = [self.key_schedule_core(self.keys[i-1][3], i) ^ self.keys[i-1][0]]
			for j in range(1, 4):
				new_key.append(self.keys[i-1][j] ^ new_key[j-1])
			self.keys.append(new_key)

	def encrypt(self, data):
		'''Encrypts some data in ECB mode.'''
		out = b''
		while data:
			out += self.encrypt_block_ecb(data[:BLOCK_SIZE])
			data = data[BLOCK_SIZE:]
		return out

	def decrypt(self, data):
		'''Decrypts some data in ECB mode.'''
		if len(data) % self.block_size:
			raise ValueError('Data is not aligned to block size!')
		out = b''
		while data:
			out += self.decrypt_block_ecb(data[:BLOCK_SIZE])
			data = data[BLOCK_SIZE:]
		return out

	def encrypt_block_ecb(self, block):
		"""Encrypt a single 128-bit block in ECB mode."""
		words = list(up('>IIII', self.pad_block(block)))
		for i in range(len(words)):
			words[i] ^= self.keys[0][i]
		for rnd in range(1, self.num_rounds + 1):
			for i in range(len(words)):
				words[i] = self.send_through_sbox(words[i], self.sbox_enc)
			words = self.shift_columns(words)
			if rnd != self.num_rounds:
				words = self.mix_columns(words)
			for i in range(len(words)):
				words[i] ^= self.keys[rnd][i]
		return pk('>IIII', words[0], words[1], words[2], words[3])

	def decrypt_block_ecb(self, block):
		"""Decrypt a single 128-bit block in ECB mode."""
		assert(len(block) == self.block_size)
		words = list(up('>IIII', block))
		for rnd in range(self.num_rounds, 0, -1):
			for i in range(len(words)):
				words[i] ^= self.keys[rnd][i]
			if rnd != self.num_rounds:
				words = self.unmix_columns(words)
			words = self.unshift_columns(words)
			for i in range(len(words)):
				words[i] = self.send_through_sbox(words[i], self.sbox_dec)
		for i in range(len(words)):
			words[i] ^= self.keys[0][i]
		return pk('>IIII', words[0], words[1], words[2], words[3])

	# Helper functions
	def rotate_op(self, word):
		'''Rotate left operation on 32-bit word.'''
		return ((word & 0xFFFFFF) << 8) | ((word & 0xFF000000) >> 24)

	def rcon_op(self, i):
		'''Round constant operation - returns rcon value for round i.'''
		assert(0 <= i and i < len(self.rcon_table))
		return self.rcon_table[i]

	def send_through_sbox(self, word, sbox=sbox_enc):
		'''Send a 32-bit word through S-box substitution.'''
		return (sbox[((word & (0xFF << 0x18)) >> 0x18)] << 0x18) | \
			(sbox[((word & (0xFF << 0x10)) >> 0x10)] << 0x10) | \
			(sbox[((word & (0xFF << 0x08)) >> 0x08)] << 0x08) | \
			(sbox[((word & (0xFF << 0x00)) >> 0x00)] << 0x00)

	def shift_columns(self, words):
		'''Perform AES row shifting (ShiftRows) on 4 words.'''
		return [
			(words[0] & 0xFF000000) | (words[1] & 0xFF0000) | (words[2] & 0xFF00) | (words[3] & 0xFF),
			(words[1] & 0xFF000000) | (words[2] & 0xFF0000) | (words[3] & 0xFF00) | (words[0] & 0xFF),
			(words[2] & 0xFF000000) | (words[3] & 0xFF0000) | (words[0] & 0xFF00) | (words[1] & 0xFF),
			(words[3] & 0xFF000000) | (words[0] & 0xFF0000) | (words[1] & 0xFF00) | (words[2] & 0xFF),
		]

	def unshift_columns(self, words):
		'''Perform inverse AES row shifting (InvShiftRows) on 4 words.'''
		return [
			(words[0] & 0xFF000000) | (words[3] & 0xFF0000) | (words[2] & 0xFF00) | (words[1] & 0xFF),
			(words[1] & 0xFF000000) | (words[0] & 0xFF0000) | (words[3] & 0xFF00) | (words[2] & 0xFF),
			(words[2] & 0xFF000000) | (words[1] & 0xFF0000) | (words[0] & 0xFF00) | (words[3] & 0xFF),
			(words[3] & 0xFF000000) | (words[2] & 0xFF0000) | (words[1] & 0xFF00) | (words[0] & 0xFF),
		]

	def mix_columns(self, words):
		'''Perform MixColumns transformation on all 4 columns.'''
		return [
			self.mix_column(words[0], self.mix_mults),
			self.mix_column(words[1], self.mix_mults),
			self.mix_column(words[2], self.mix_mults),
			self.mix_column(words[3], self.mix_mults),
		]

	def unmix_columns(self, words):
		'''Perform inverse MixColumns transformation on all 4 columns.'''
		return [
			self.mix_column(words[0], self.unmix_mults),
			self.mix_column(words[1], self.unmix_mults),
			self.mix_column(words[2], self.unmix_mults),
			self.mix_column(words[3], self.unmix_mults),
		]

	def mix_column(self, word, mults):
		'''Perform MixColumns on a single column using given multipliers.'''
		return ((self.mix(word, mults[0])) << 0x18) | \
			((self.mix(word, mults[1])) << 0x10) | \
			((self.mix(word, mults[2])) << 0x08) | \
			((self.mix(word, mults[3])) << 0x00)

	def mix(self, word, mix):
		'''Mix a word using Galois field multiplication with lookup table.'''
		return (self.mult_table[mix[0]][((word >> 0x18) & 0xFF)]) ^ \
			(self.mult_table[mix[1]][((word >> 0x10) & 0xFF)]) ^ \
			(self.mult_table[mix[2]][((word >> 0x08) & 0xFF)]) ^ \
			(self.mult_table[mix[3]][((word >> 0x00) & 0xFF)]) & 0xFF

	def key_schedule_core(self, word, i, sbox=sbox_enc):
		'''Perform key schedule core operation (SubWord, RotWord, Xor with rcon).'''
		return self.send_through_sbox(self.rotate_op(word), sbox) ^ (self.rcon_op(i) << 0x18)

	def pad_block(self, block):
		'''Pad a block with PKCS7 padding to BLOCK_SIZE.'''
		assert(len(block) <= self.block_size)
		num_pad = self.block_size - len(block)
		right = (chr(num_pad) * num_pad).encode()
		return block + right
