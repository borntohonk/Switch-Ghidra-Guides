#!/usr/bin/env python3
"""
emummc_h.py - Extract FS offsets from Switch KIP1 for emummc (adjusted last for: 22.0.0)
...
FS_OFFSET_LOCK_MUTEX_PATTERN = "fd7bbea9f30b00f9fd03009108044039f30300aa1f05007181000054606200913406009460000037"
FS_OFFSET_UNLOCK_MUTEX_PATTERN = "080840b908050071080800b940000054c0035fd6006000911206001400000000"
...
def produce_emummc_h(kip_path: str, firmware: str, prefix: str = "", version: str = None):
    ...
    fname = f"{firmware}_exfat" if prefix else firmware
    guard = f"__FS_{fname}_H__".upper()

    atm_dir = os.path.join(".", "atmosphere")
    atm_path = os.path.join(atm_dir, "emummc", "source", "FS", "offsets", f"{fname}.h")

    use_fallback = False
    if not os.path.isdir(atm_dir):
        print("[WARN] ./atmosphere not found — writing to output/ instead")
        use_fallback = True
    elif not os.listdir(atm_dir):
        print("[WARN] ./atmosphere is empty — writing to output/ instead")
        use_fallback = True
    elif os.path.exists(atm_path):
        print(f"[WARN] {atm_path} already exists — writing to output/ instead")
        use_fallback = True

    if use_fallback:
        if not version:
            print("[WARN] no version provided — cannot determine fallback path, skipping")
            return
        out_path = os.path.join(".", "output", version, f"{fname}.h")
    else:
        out_path = atm_path
    ...
    lines.append("")
    lines.append("// Accessor vtable getters")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_ACCESSOR_GC   0x{off_gc:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_ACCESSOR_SD   0x{off_sd:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_ACCESSOR_NAND 0x{off_nand:X}")
    lines.append("")
    lines.append("// Hooks")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_WRAPPER_READ  0x{off_read:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_WRAPPER_WRITE 0x{off_write:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}RTLD                0x{off_rtld:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}RTLD_DESTINATION    {rtld_c}")
    lines.append("")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}CLKRST_SET_MIN_V_CLK_RATE 0x{off_clk:X}")
    lines.append("")
    lines.append("// Misc funcs")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}LOCK_MUTEX          0x{off_lock:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}UNLOCK_MUTEX        0x{off_unlock:X}")
    lines.append("")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_WRAPPER_CONTROLLER_OPEN  0x{off_ctrl_open:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_WRAPPER_CONTROLLER_CLOSE 0x{off_ctrl_close:X}")
    lines.append("")
    lines.append("// Misc Data")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SD_MUTEX            0x{sd_mutex:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}NAND_MUTEX          0x{nand_mutex:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}ACTIVE_PARTITION    0x{active_part:X}")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SDMMC_DAS_HANDLE    0x{das_handle:X}")
    lines.append("")
    lines.append("// NOPs")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}SD_DAS_INIT         0x{off_das_init:X}")
    lines.append("")
    lines.append("// Nintendo Paths")
    lines.append(f"#define FS_OFFSET_{firmware}_{prefix}NINTENDO_PATHS \\")
    if n_paths:
        lines.append("{ \\")
        for reg, off in n_paths:
            lines.append(f"    {{.opcode_reg = {reg}, .adrp_offset = 0x{off:08X}, .add_rel_offset = 0x00000004}}, \\")
        lines.append("    {.opcode_reg = 0, .adrp_offset = 0, .add_rel_offset = 0}, \\")
        lines.append("}")
    else:
        lines.append("// No Nintendo paths found (check patterns or KIP variant)")
    lines.append("")
...
def main():
    if len(sys.argv) not in (4, 5):
        print("Usage: python emummc_h.py <kip1_file> <version e.g. 2100> <prefix e.g. '' or EXFAT_> [dotted_version e.g. 22.5.0]")
        sys.exit(1)
    ver = sys.argv[4] if len(sys.argv) == 5 else None
    produce_emummc_h(sys.argv[1], sys.argv[2], sys.argv[3], version=ver)
