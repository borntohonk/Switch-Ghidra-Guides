...
class BinFile(object):
    ...
    def read(self, arg):
        if isinstance(arg, str):
            ...
            if isinstance(arg, int) and len(out) != arg:
...
class NxoFileBase(object):
    ...
    def get_dynstr(self, o):
        raw = self.dynstr[o:self.dynstr.index(b'\0', o)]
    ...
    def get_path_or_name(self):
        ...
        if path is None:
            strs = re.findall(rb'[a-z]:[\\/][ -~]{5,}\.n[rs]s', as_string, flags=re.IGNORECASE)
            if strs:
                return strs[-1]
...
class NsoFile(NxoFileBase):
    def __init__(self, fileobj):
        ...
        super().__init__(BinFile(BytesIO(full)), (tloc, tsize, rloc, rsize, dloc, dsize))
...
class NroFile(NxoFileBase):
    def __init__(self, fileobj):
        ...
        super().__init__(filecopy, (tloc, tsize, rloc, rsize, dloc, dsize))

def kip1_blz_decompress(compressed):
    compressed_size, init_index, uncompressed_addl_size = struct.unpack('<III', compressed[-0xC:])
    decompressed = bytearray(compressed) + bytearray(uncompressed_addl_size)
